...
const iconMap: Record<IconType, React.ComponentType<any>> = {
    [IconType.AddPerson]: AddPersonIcon,
    [IconType.Dial]: DialIcon,
...
export default function Icon({ type, props }: IconProps) {
    const Compnent = iconMap[type];

    if (!Compnent) return <div>Error Icon</div>;

    return <Compnent {...props} />;
}

function AddPersonIcon() {
    return (<svg className="w-6 h-6 text-white dark:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="currentColor" viewBox="0 0 24 24">
